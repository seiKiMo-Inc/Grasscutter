...
public final class NetworkSession extends KcpTunnel implements KcpHandler {
    ...
    public void send(BasePacket<?, ?> packet) {
        ...
        if (!NetworkConstants.LOG_BLACKLIST.contains(packetId)) {
            ...
            Log.debug(
                    new Text(
                            new TextContainer(
                                    "network.packet.outbound", name, packetId, this.getPrettyAddress(true))));
        }
    ...
    public String getPrettyAddress(boolean withPort) {
        ...
        return address.getAddress().getHostAddress() + (withPort ? ":" + address.getPort() : "");
    }
    ...
    public void onConnect() {
        // Log the connection.
        Log.debug(new TextContainer("server.game.client.connected", this.getPrettyAddress(true)));
    }

    @Override
    public void onDisconnect() {
        // Log the disconnection.
        Log.debug(new TextContainer("server.game.client.disconnected", this.getPrettyAddress(true)));
    }
    ...
    public void onMessage(byte[] data) {
        ...
        CryptoUtils.performXor(
                data, this.isEncrypted() ? KeyType.ENCRYPT.getKey() : KeyType.DISPATCH.getKey());
        ...
        try {
            while (buffer.isReadable()) {
                ...
                // Handle the packet.
                PacketHandler.handlePacket(this, packetId, headData, packetData);
            }
            ...
            // Log the exception.
            Log.error(
                    new TextContainer("server.game.client.error", this.getPrettyAddress(true)), exception);
        }
    }
}

...
public interface PrimitiveUtils {
    ...
    Map<Type, Function<String, Object>> PRIMITIVE_PARSERS =
            Map.ofEntries(
    ...
    /**
     * Splits a string by a separator. Does not perform a Regex split.
     *
    ...
    static List<String> split(String string, int separator) {
        ...
        for (var next = string.indexOf(separator); next > 0; next = string.indexOf(separator, start)) {
        ...
        if (start < string.length()) output.add(string.substring(start));

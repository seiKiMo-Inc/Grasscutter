...
@AllArgsConstructor
@Getter public enum ActionReason {
	None(0),
	QuestItem(1),
	QuestReward(2),
	Trifle(3),
	...
	static {
		// Cache the values.
		Stream.of(ActionReason.values()).forEach(entry -> {
	...
	/**
	 * Fetches the ActionReason for the given value.
	 *
	 * @param value The value to fetch.
	 * @return The ActionReason.
	 */
	public static ActionReason fetch(int value) {
		return map.getOrDefault(value, None);
	}

	/**
	 * Fetches the ActionReason for the given name.
	 *
	 * @param name The name to fetch.
	 * @return The ActionReason.
	 */
	public static ActionReason fetch(String name) {
